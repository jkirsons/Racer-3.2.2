...
var target_node : VehicleKinematicBody
var last_lookat

func _ready():
	target_node = get_node(target_object) as VehicleKinematicBody
...
func _physics_process(delta):
	...
	var new_target_distance = target_distance + 1 * target_node.speed / target_node.max_speed
	
	# ignore y
	target_vector.y = 0.0
	
	if (target_vector.length() > new_target_distance):
		# limit vector to targe_distance
		target_vector = target_vector.normalized() * new_target_distance
